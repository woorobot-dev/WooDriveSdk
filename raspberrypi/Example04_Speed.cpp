...
static const uint8_t MOTION_MODE = 116;
// static const uint8_t MOTION_MODE = 117;

static const float SUB_TARGET = 10.0f;
static const float MAIN_TARGET_RPM = 100.0f;
...
static bool monitorStatus(unsigned long durationMs)
{
    ...
    while (std::chrono::steady_clock::now() < deadline)
    {
        ...
        if (ok1 && ok2)
        {
            ...
            {
                printTitle("RUN FAIL");
                std::cout << "Fault detected during motor run.\n";
                return false;
            }
        }
        else
...
int main()
{
    ...
    if (!setRunReady())
    {
        std::cout << "Run ready setup failed before RUN 2.\n";
        return 0;
    }
    ...
    if (!setRunReady())
    {
        std::cout << "Run ready setup failed before RUN 3.\n";
        return 0;
    }
    ...
    if (!setRunReady())
    {
        std::cout << "Run ready setup failed before RUN 4.\n";
        return 0;
    }
    ...
    if (!setRunReady())
    {
        std::cout << "Run ready setup failed before RUN 5.\n";
        return 0;
    }
